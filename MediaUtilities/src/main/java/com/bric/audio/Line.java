 ...
 * <p>
 * A line can have controls, such as gain, pan, and reverb. The controls themselves are instances of classes that extend
 * the base <code>{@link Control}</code> class. The <code>Line</code> interface provides two accessor methods for
 * obtaining the line's controls: <code>{@link #getControls getControls}</code> returns the entire set, and
 * <code>{@link #getControl getControl}</code> returns a single control of specified type.
 * <p>
 * Lines exist in various states at different times. When a line opens, it reserves system resources for itself, and
 * when it closes, these resources are freed for other objects or applications. The <code>{@link #isOpen()}</code>
 * method lets you discover whether a line is open or closed. An open line need not be processing data, however. Such
 * processing is typically initiated by subinterface methods such as
 ...
 * <code>{@link LineListener}</code> interface, which consists of the single method
 * <code>{@link LineListener#update update}</code>. This method will be invoked when a line opens and closes (and, if
 * it's a {@link DataLine}, when it starts and stops).
 * <p>
 * An object can be registered to listen to multiple lines. The event it receives in its <code>update</code> method will
 * specify which line created the event, what type of event it was (<code>OPEN</code>, <code>CLOSE</code>,
 * <code>START</code>, or <code>STOP</code>), and how many sample frames the line had processed at the time the event
 * occurred.
 * <p>
...
public interface Line {
	...
	/**
	 * Opens the line, indicating that it should acquire any required system resources and become operational. If this
	 * operation succeeds, the line is marked as open, and an <code>OPEN</code> event is dispatched to the line's
	 * listeners.
	 * <p>
	 * Note that some lines, once closed, cannot be reopened. Attempts to reopen such a line will always result in an
	 ...
	 * <code>DataLine</code> must be opened with a particular format and buffer size. Such lines should provide a
	 * mechanism for configuring these properties, such as an additional <code>open</code> method or methods which allow
	 ...
	 * <code>{@link SourceDataLine}</code> and <code>{@link TargetDataLine}</code> objects, this means that the line is
	 * opened with default settings. For a <code>{@link Clip}</code>, however, the buffer size is determined when data
	 * is loaded. Since this method does not allow the application to specify any data to load, an
	 * IllegalArgumentException is thrown. Therefore, you should instead use one of the <code>open</code> methods
	...
	/**
	 * A <code>Line.Info</code> object contains information about a line. The only information provided by
	 * <code>Line.Info</code> itself is the Java class of the line. A subclass of <code>Line.Info</code> adds other
	 * kinds of information about the line. This additional information depends on which <code>Line</code> subinterface
	 * is implemented by the kind of line that the <code>Line.Info</code> subclass describes.
	 * <p>
	...
	public static class Info {
		...
		/**
		 * Indicates whether the specified info object matches this one. To match, the specified object must be
		 * identical to or a special case of this one. The specified info object must be either an instance of the same
		 * class as this one, or an instance of a sub-type of this one. In addition, the attributes of the specified
		...
		public boolean matches(Info info) {
			...
			// dataLine.matches(targetDataLine) == true: targetDataLine is
			// always dataLine
			...
			// => prospective_match may be other things as well, but it is at
			// least a targetDataLine
			// targetDataLine defines the requirements which prospective_match
			// must meet.

			// "if this Class object represents a declared class, this method
			// returns
			// true if the specified Object argument is an instance of the
			// represented
			...
			// this_class.isInstance(that_object) => that object can by cast to
			// this class
			// => that_object's class may be a subtype of this_class
			// => that may be more specific (subtype) of this

			// "If this Class object represents an interface, this method
			// returns true
		...
		public String toString() {
			...
			if (index != -1) {
				finalString = initialString.substring(0, index)
						+ initialString.substring((index + fullPackagePath.length()), initialString.length());
			} else {

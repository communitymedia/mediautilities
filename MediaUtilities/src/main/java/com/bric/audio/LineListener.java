...
/**
 * Instances of classes that implement the <code>LineListener</code> interface can register to receive events when a
 * line's status changes.
 * 
 * @author Kara Kytle
 * 
 * @see Line
...
public interface LineListener extends java.util.EventListener {
	...
	/**
	 * Informs the listener that a line's state has changed. The listener can then invoke <code>LineEvent</code> methods

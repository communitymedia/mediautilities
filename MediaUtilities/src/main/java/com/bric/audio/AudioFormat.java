 ...
 * <p>
 * Every data line has an audio format associated with its data stream. The audio format of a source (playback) data
 * line indicates what kind of data the data line expects to receive for output. For a target (capture) data line, the
 * audio format specifies the kind of the data that can be read from the line. Sound files also have audio formats, of
 * course. The <code>{@link AudioFileFormat}</code> class encapsulates an <code>AudioFormat</code> in addition to other,
 * file-specific information. Similarly, an <code>{@link AudioInputStream}</code> has an <code>AudioFormat</code>.
 * <p>
 * The <code>AudioFormat</code> class accommodates a number of common sound-file encoding techniques, including
 * pulse-code modulation (PCM), mu-law encoding, and a-law encoding. These encoding techniques are predefined, but
 * service providers can create new encoding types. The encoding that a specific format uses is named by its
 * <code>encoding</code> field.
 * <p>
 * In addition to the encoding, the audio format includes other properties that further specify the exact arrangement of
 * the data. These include the number of channels, sample rate, sample size, byte order, frame rate, and frame size.
 * Sounds may have different numbers of audio channels: one for mono, two for stereo. The sample rate measures how many
 * "snapshots" (samples) of the sound pressure are taken per second, per channel. (If the sound is stereo rather than
 * mono, two samples are actually measured at each instant of time: one for the left channel, and another for the right
 * channel; however, the sample rate still measures the number per channel, so the rate is the same regardless of the
 * number of channels. This is the standard use of the term.) The sample size indicates how many bits are used to store
 * each snapshot; 8 and 16 are typical values. For 16-bit samples (or any other sample size larger than a byte), byte
 * order is important; the bytes in each sample are arranged in either the "little-endian" or "big-endian" style. For
 * encodings like PCM, a frame consists of the set of samples for all channels at a given point in time, and so the size
 * of a frame (in bytes) is always equal to the size of a sample (in bytes) times the number of channels. However, with
 ...
 * format specifications, like the bit rate for compressed formats. Properties are mainly used as a means to transport
 * additional information of the audio format to and from the service providers. Therefore, properties are ignored in
 ...
 * <td><code>true</code>, if the file is encoded in variable bit rate (VBR)</td>
 * </tr>
 * <tr>
 * <td>&quot;quality&quot;</td>
 * <td>{@link java.lang.Integer Integer}</td>
...
public class AudioFormat {
	 ...
	 * @param bigEndian
	 *            indicates whether the data for a single sample is stored in big-endian byte order (<code>false</code>
	 *            means little-endian)
	 */
	...
	public AudioFormat(float sampleRate, int sampleSizeInBits, int channels, boolean signed, boolean bigEndian) {
		...
		this(
				...
				sampleSizeInBits,
				channels,
				(channels == AudioSystem.NOT_SPECIFIED || sampleSizeInBits == AudioSystem.NOT_SPECIFIED) ? AudioSystem.NOT_SPECIFIED
	...
	/**
	 * Obtains the sample rate. For compressed formats, the return value is the sample rate of the uncompressed audio
	 * data. When this AudioFormat is used for queries (e.g.
	 * {@link AudioSystem#isConversionSupported(AudioFormat, AudioFormat) AudioSystem.isConversionSupported}) or
	 * capabilities (e.g. {@link DataLine.Info#getFormats() DataLine.Info.getFormats}), a sample rate of
	...
	/**
	 * Obtains the size of a sample. For compressed formats, the return value is the sample size of the uncompressed
	 * audio data. When this AudioFormat is used for queries (e.g.
	 * {@link AudioSystem#isConversionSupported(AudioFormat, AudioFormat) AudioSystem.isConversionSupported}) or
	 * capabilities (e.g. {@link DataLine.Info#getFormats() DataLine.Info.getFormats}), a sample size of
	...
	/**
	 * Obtains the frame size in bytes. When this AudioFormat is used for queries (e.g.
	 * {@link AudioSystem#isConversionSupported(AudioFormat, AudioFormat) AudioSystem.isConversionSupported}) or
	...
	/**
	 * Indicates whether this format matches the one specified. To match, two formats must have the same encoding, the
	 * same number of channels, and the same number of bits per sample and bytes per frame. The two formats must also
	 * have the same sample rate, unless the specified format has the sample rate value
	...
	public boolean matches(AudioFormat format) {
		...
		if (format.getEncoding().equals(getEncoding())
				...
				&& (format.getChannels() == getChannels()
						...
						&& ((format.getFrameRate() == (float) AudioSystem.NOT_SPECIFIED) || (format.getFrameRate() == getFrameRate())) && ((format
	...
	/**
	 * The <code>Encoding</code> class names the specific type of data representation used for an audio stream. The
	 * encoding includes aspects of the sound format other than the number of channels, sample rate, sample size, frame
	 ...
	 * representation of the sound waveform. With PCM, the number stored in each sample is proportional to the
	 * instantaneous amplitude of the sound pressure at that point in time. The numbers are frequently signed or
	 * unsigned integers. Besides PCM, other encodings include mu-law and a-law, which are nonlinear mappings of the
	 * sound amplitude that are often used for recording speech.
	 * <p>
	 * You can use a predefined encoding by referring to one of the static objects created by this class, such as
	 * PCM_SIGNED or PCM_UNSIGNED. Service providers can create new encodings, such as compressed audio formats or
	 * floating-point PCM samples, and make these available through the <code>{@link AudioSystem}</code> class.
	 * <p>

 ...
 * <p>
 * The implementation stores single bits as a word in the buffer. If a bit is set, the corresponding word in the buffer
 * will be non-zero. If a bit is clear, the corresponding word is zero. Although this may seem waseful, this can be a
...
final class BitReserve {
	 ...
	 * BUFSIZE = 4096 = 2^12, so // buf_byte_idx%BUFSIZE == buf_byte_idx & 0xfff val = buf[buf_byte_idx & BUFSIZE_MASK]
	 * & putmask[buf_bit_idx]; buf_bit_idx--; val = val >>> buf_bit_idx; return val; }
	 */
	/**
	 ...
	 * buf[buf_byte_idx & BUFSIZE_MASK]; } // save total number of bits returned len = buf_bit_idx; buf_bit_idx = 0; int
	 * b = current; int count = len-1; while (count >= 0) { out[count--] = (b & 0x1); b >>>= 1; } totbit += len; return
	 * len; }
	 */

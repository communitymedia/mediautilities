...
public class GainControl implements GCConstants {
	...
	/**
	 * gain compensation and overlap-add: - the gain control function is calculated - the gain control function applies
	 * to IMDCT output samples as a another IMDCT window - the reconstructed time domain signal produces by overlap-add
	 */
	...
	/**
	 * calculates a fragment modification function the interpolated gain value between the gain values of two gain
	 * change positions is calculated by the formula: f(a,b,j) = 2^(((8-j)log2(a)+j*log2(b))/8)
	 */

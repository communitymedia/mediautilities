...
class HighlightView {
	...
	private void init() {
		...
		// mResizeDrawableDiagonal = resources.getDrawable(R.drawable.camera_crop_autocrop_indicator);
	}
	...
	protected void draw(Canvas canvas) {
		...
		if (!hasFocus()) {
			...
			// if (mCircle) {
			// int diagWidth = mResizeDrawableDiagonal.getIntrinsicWidth();
			// int diagHeight = mResizeDrawableDiagonal.getIntrinsicHeight();
			//
			// int d = (int) Math.round(Math.cos(/* 45deg */Math.PI / 4D) * (mDrawRect.width() / 2D));
			// int x = mDrawRect.left + (mDrawRect.width() / 2) + d - diagWidth / 2;
			// int y = mDrawRect.top + (mDrawRect.height() / 2) - d - diagHeight / 2;

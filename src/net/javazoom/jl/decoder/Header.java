...
public final class Header {
	...
	void read_header(Bitstream stream, Crc16[] crcp) throws BitstreamException {
		...
		if (h_sample_frequency == FOURTYFOUR_POINT_ONE) {
			/*
			 * if (offset == null) { int max = max_number_of_frames(stream); offset = new int[max]; for(int i=0; i<max;
			 * i++) offset[i] = 0; } // E.B : Investigate more int cf = stream.current_frame(); int lf =
			 * stream.last_frame(); if ((cf > 0) && (cf == lf)) { offset[cf] = offset[cf-1] + h_padding_bit; } else {
		...
		try {
			...
			if (xing.equals(new String(tmp))) {
				...
				if ((flags[3] & (byte) (1 << 0)) != 0) {
					...
					h_vbr_frames = (tmp[0] << 24) & 0xFF000000 | (tmp[1] << 16) & 0x00FF0000 | (tmp[2] << 8)
				...
				if ((flags[3] & (byte) (1 << 1)) != 0) {
					System.arraycopy(firstframe, offset + length, tmp, 0, tmp.length);
					h_vbr_bytes = (tmp[0] << 24) & 0xFF000000 | (tmp[1] << 16) & 0x00FF0000 | (tmp[2] << 8)
							& 0x0000FF00 | tmp[3] & 0x000000FF;
					length += 4;
				}
				...
				if ((flags[3] & (byte) (1 << 3)) != 0) {
					System.arraycopy(firstframe, offset + length, tmp, 0, tmp.length);
					h_vbr_scale = (tmp[0] << 24) & 0xFF000000 | (tmp[1] << 16) & 0x00FF0000 | (tmp[2] << 8)
							& 0x0000FF00 | tmp[3] & 0x000000FF;
					length += 4;
				}
	...
	public static final String bitrate_str[][][] = {
...

			{
					{ "free format", "32 kbit/s", "64 kbit/s", "96 kbit/s", "128 kbit/s", "160 kbit/s", "192 kbit/s",
							"224 kbit/s", "256 kbit/s", "288 kbit/s", "320 kbit/s", "352 kbit/s", "384 kbit/s",

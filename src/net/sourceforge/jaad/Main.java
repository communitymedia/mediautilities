...
public class Main {
	...
	private static void decodeMP4(String in, String out) throws Exception {
		...
		try {
			...
			wav = new WaveFileWriter(new File(out), track.getSampleRate(), track.getChannelCount(),
					track.getSampleSize());
	...
	private static void decodeAAC(String in, String out) throws IOException {
		...
		try {
			...
			while (true) {
				...
				if (wav == null)
					wav = new WaveFileWriter(new File(out), buf.getSampleRate(), buf.getChannels(),
							buf.getBitsPerSample());
				wav.write(buf.getData());
			}
		} finally {
			if (wav != null)

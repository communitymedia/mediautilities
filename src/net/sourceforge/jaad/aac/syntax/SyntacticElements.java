...
public class SyntacticElements implements Constants {
		...
		if (!config.getProfile().isErrorResilientProfile()) {
			...
			switch (config.getChannelConfiguration()) {
				...
				case CHANNEL_CONFIG_SEVEN_PLUS_ONE:
					...
					break;
				default:
					throw new AACException("unsupported channel configuration for error resilience: "
	...
	private int processSingle(SCE_LFE scelfe, FilterBank filterBank, int channel, Profile profile, SampleFrequency sf)
		...
		if (sbrPresent) {
			...
			if (sbr.isPSUsed()) {
				...
				scelfe.getSBR().process(data[channel], null, false);
		}
		return chs;
	}
	...
	private void processPair(CPE cpe, FilterBank filterBank, int channel, Profile profile, SampleFrequency sf)
		...
		// SBR
		if (sbrPresent) {
			// if(data[channel].length==config.getFrameLength()) LOGGER.log(Level.WARNING,
			// "SBR data present, but buffer has normal size!");

...
/**
 * The Track Reference Box provides a reference from the containing track to another track in the presentation. These
 * references are typed. A 'hint' reference links from the containing hint track to the media data that it hints. A
 * content description reference 'cdsc' links a descriptive or metadata track to the content which it describes.
 * 
...
public class TrackReferenceBox extends BoxImpl {
	 ...
	 * <li>'hind': this track depends on the referenced hint track, i.e., it should only be used if the referenced hint
	 * track is used.</li>
	 * 
	...
	/**
	 * The track IDs are integers that provide a reference from the containing track to other tracks in the
	 * presentation. Track IDs are never re-used and cannot be equal to zero.
	 * 

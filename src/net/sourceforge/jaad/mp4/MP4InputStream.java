...
public class MP4InputStream {
	 ...
	 * @throws IOException If the end of the stream is detected, the input stream has been closed, or if some other I/O
	 *             error occurs.
	 * @throws IndexOutOfBoundsException if <code>n</code> is not in the range [1...8] inclusive.
	 */
	...
	public String readUTFString(int max, String encoding) throws IOException {
		return StringUtilities.byteToString(readTerminated(max, 0), Charset.forName(encoding));
	}
	...
	public String readUTFString(int max) throws IOException {
		...
		return StringUtilities.byteToString(b2, Charset.forName((i == BYTE_ORDER_MASK) ? UTF16 : UTF8));
	}
	 ...
	 * @throws IOException If the end of the stream is detected, the input stream has been closed, or if some other I/O
	 *             error occurs.
	 * @throws IllegalArgumentException if the total number of bits (m+n) is not a multiple of eight
	 */

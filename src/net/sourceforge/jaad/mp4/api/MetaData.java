...
public class MetaData {
	...
	private static final String[] STANDARD_GENRES = {
			...
			"new age", "oldies", "other", "pop", "r and b", "rap", "reggae", "rock", "techno", "industrial",
			...
			"eurodance", "dream", "southern rock", "comedy", "cult", "gangsta", "top ", "christian rap", "pop funk",
			"jungle", "native american", "cabaret", "new wave", "psychedelic", "rave", "showtunes", "trailer", "lo fi",
			"tribal", "acid punk", "acid jazz", "polka", "retro", "musical", "rock and roll",
			// winamp extension
	...
	private static final String[] NERO_TAGS = { "artist", "title", "album", "track", "totaltracks", "year", "genre",
			"disc", "totaldiscs", "url", "copyright", "comment", "lyrics", "credits", "rating", "label", "composer",
			"isrc", "mood", "tempo" };
	...
	/*
	 * moov.udta: -3gpp boxes -meta --ilst --tags --meta (no container!) --tseg ---tshd
	 */
	...
	private void parse3GPPData(Box udta) {
		...
		if (udta.hasChild(BoxTypes.THREE_GPP_LOCATION_INFORMATION_BOX))
			put(Field.LOCATION,
					((ThreeGPPLocationBox) udta.getChild(BoxTypes.THREE_GPP_LOCATION_INFORMATION_BOX)).getPlaceName());
		...
		if (udta.hasChild(BoxTypes.THREE_GPP_RECORDING_YEAR_BOX)) {
			...
			try {
				...
				Logger.getLogger("MP4 API").log(Level.INFO, "unable to parse 3GPP metadata: recording year value: {0}",
						value);
			}
		}
	...
	private void parseITunesMetaData(Box ilst) {
		...
		for (Box box : boxes) {
			...
			if (l == BoxTypes.ARTIST_NAME_BOX)
				...
				byte[] b = data.getData();
				put(Field.TRACK_NUMBER, Integer.valueOf(b[3]));
				put(Field.TOTAL_TRACKS, Integer.valueOf(b[5]));

...
public class SVGParser {
	...
	/**
	 * Elliptical arc implementation based on the SVG specification notes Adapted from the Batik library (Apache-2
	 * license) by SAU
	 */
	...
	private static class SVGHandler extends DefaultHandler {
		...
		RectF bounds = null;
		RectF limits = new RectF(Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY,
		...
		public void endDocument() {
			...
			/*
			 * String s = parsed.toString(); if (s.endsWith("</svg>")) { s = s + ""; Log.d(TAG, s); }
			 */
		}
		...
		private void pushTransform(Attributes atts) {
			...
			/*
			 * final Matrix m = canvas.getMatrix(); m.postConcat(matrix); canvas.setMatrix(m);
			 */
		...
		public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
			...
			if (localName.equals("svg")) {
				...
				if (gradient != null) {
					...
					if (colorStyle != null) {
						color = parseColorString(colorStyle);
				...
				if (localName.equals("use")) {
					...
					for (int i = 0; i < atts.getLength(); i++) {
						...
						if (!"x".equals(attrQName) && !"y".equals(attrQName) && !"width".equals(attrQName)
		...
		public void endElement(String namespaceURI, String localName, String qName) {
			...
			if (localName.equals("linearGradient")) {
				if (gradient.id != null) {
					...
					LinearGradient g = new LinearGradient(gradient.x1, gradient.y1, gradient.x2, gradient.y2, colors,
	...
	public static Integer parseColorString(String v) {
	...
	private static int hex3Tohex6(int x) {

...
/**
 * This filtered stream places an initial limit on the number of bytes that can be read. Once this limit is reached, all
 * methods in this object return -1 indicating we're at the EOF, although the underlying InputStream is not touched.
 * <P>
 * This object is useful when parsing files where specific blocks of a file have a predetermined size: with this object
 * you can guarantee you read only a fixed number of bytes. So if data inside the block is corrupt, or if you want to
 * loosely guard how you read each block, this object will make sure you don't read too far.
 * <P>
...
public class GuardedInputStream extends MeasuredInputStream {
	 ...
	 * <P>
	 * This number has nothing to do with the number of bytes that are actually remaining in the underlying
	 * <code>InputStream</code>. For example, if this <code>GuardedInputStream</code> was designed to read at most 1000
	 * bytes, then this method may return 1000 -- even though there may only be 500 bytes available in the underlying

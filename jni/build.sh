...
# note: shared library does not actually work; this is more of an experiment than an option
...
# create the custom NDK toolchain that we'll use to build
...
# create the Android.mk file, using the appropriate version for static or shared library
if [ "$USE_SHARED_LIBRARY" != "yes" ]; then
...
# finally, build the wrapper library
